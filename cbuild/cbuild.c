...
#define pPushStr(array, str) pPushBytes((array), (str), sizeof(str) - 1)
    ...
    case MODE_RELEASE: pPushStr(command, "-O2 -DRELEASE -DNDEBUG "); break;
    case MODE_RELEASE_WITH_DEBUG: 
                  pPushStr(command, "-g -O2 -DRELEASE -DDEBUG -DREL_WITH_DEBUG "); break;
    default: break;
    }
...
#define pPushStr(array, str) pPushBytes((array), (str), sizeof(str) - 1)
    ...
    switch(ctx->mode) {
    case MODE_DEBUG:   pPushStr(command, "-g3 -O0 -Wall -Wextra -DDEBUG ");  break;
    case MODE_RELEASE: pPushStr(command, "-O2 -DRELEASE -DNDEBUG "); break;
    case MODE_RELEASE_WITH_DEBUG: 
        pPushStr(command, "-g -O2 -DRELEASE -DDEBUG -DREL_WITH_DEBUG "); break;
    default: break;
    ...
    if (ctx->type & UNITY_BUILD) {
        ...
        case EXECUTABLE: {
                ...
                if (ctx->compiler == EMCC) {
                    if (ctx->html_output)
                         pPushStr(next_command, ".html");
                    else pPushStr(next_command, ".js");
                } else {
#if defined(CBUILD_WINDOWS)
